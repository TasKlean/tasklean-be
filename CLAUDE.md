...
- **Audit trail**: any mutating service method on an audited aggregate must write an audit entry via `AuditLogService.record(entityType, entityId, action, message, group)` — audited aggregates are `Task`, `Group`, `GroupMember`, `Category`, `Tag`, `TaskCompletion`, plus auth/account events on `User` (register, login, logout, account deleted, profile update, and `LOGIN_FAILED` on every login rejection where the user is known). Call `record` *after* the save, inside the same `@Transactional` method so the entry commits atomically with the change (the exception is `login()`, deliberately non-transactional so failed-login entries survive the thrown `BadCredentialsException`). Use `AuditEntityType` / `AuditAction` constants — never inline strings. Pass the owning `Group` so the acting member is attributed; pass `null` only for group-less events. `record` resolves actor and IP itself from `AuditContext` — don't pass them. Entries are never updated or deleted (append-only).
