...
- **`auth`** — `AuthController` (register/login/verify/resend/refresh/logout/google endpoints), `AuthService` (credential validation, user creation), `GoogleOAuthService` (verifies a Google ID token via `GoogleIdTokenVerifier`, then finds/links/provisions the user — see `config/GoogleOAuthConfig`). DTOs: `LoginRequest`, `RegisterRequest`, `GoogleAuthRequest`, `AuthResponse` (includes `refreshToken` field).
  - **`auth/jwt`** — `JwtService` (token generation/validation, claims incl. platform `role`), `JwtAuthenticationFilter` (extracts Bearer token, builds an `AuthPrincipal` from claims — **stateless, no DB load** — and sets SecurityContext with a `ROLE_<role>` authority), `AuthPrincipal` (claims-derived principal record), `JwtAuthenticationEntryPoint` (401 JSON response). Deactivation/role changes take effect at token refresh, not per-request (bounded staleness = access-token TTL).
...
- **Flyway migrations**: `src/main/resources/db/migration/V<N>__description.sql`. Next available version is V18.
...
- **Audit trail**: any mutating service method on an audited aggregate must write an audit entry via `AuditLogService.recordEvent(entityType, entityId, action, message, group)` — audited aggregates are `Task`, `Group`, `GroupMember`, `Category`, `Tag`, `TaskCompletion`, plus auth/account events on `User` (register, login, logout, account deleted, profile update, and `LOGIN_FAILED` on every login rejection where the user is known). Call `recordEvent` *after* the save, inside the same `@Transactional` method so the entry commits atomically with the change (the exception is `login()`, deliberately non-transactional so failed-login entries survive the thrown `BadCredentialsException`). Use `AuditEntityType` / `AuditAction` constants — never inline strings. Pass the owning `Group` so the acting member is attributed; pass `null` only for group-less events. `recordEvent` resolves actor and IP itself from `AuditContext` — don't pass them. Entries are never updated or deleted (append-only).
- **Authorization (RBAC)**: enforced declaratively with Spring method security (`@EnableMethodSecurity`), not with checks scattered in services. Two role systems: **platform role** (`User.role`, in the JWT) via `@PreAuthorize("hasRole('ADMIN')")` — a `RoleHierarchy` bean makes `SUPER_ADMIN → ADMIN → USER`, so check the *lowest* role that should pass; and **group role** (`GroupMember.role`, relationship-based) via the `@groupSecurity` bean referenced in the expression (e.g. `@groupSecurity.isAdmin(#uid)`, `isMemberOfGroup(#groupId)`, `canManageMember(#id)`), which resolves the caller's active membership of the target group. Self-access uses `@accountSecurity.isSelf(#uid)`. **Every new endpoint that isn't public must carry a `@PreAuthorize`** stating who may call it (a missing one is an access-control hole — the default is only `authenticated()`). Return the *lowest* privilege that works; ADMIN is read-only across the platform, only SUPER_ADMIN does destructive/role-granting actions. Denials surface as 403 via `GlobalExceptionHandler`.
- **Rate limiting**: token-bucket (Bucket4j) in `RateLimitFilter`, in the security chain after `JwtAuthenticationFilter`. Tiers live in `RateLimitProperties` (`ratelimit.*`, overridable per env). An **authenticated** request must pass **both** a per-user and a per-IP bucket; **unauthenticated** requests are limited per IP by the endpoint tier (auth/register/refresh, else the api-per-IP backstop). Over-limit → **429** with `Retry-After`, written as raw JSON in the filter (filters run before MVC, so `GlobalExceptionHandler` doesn't see them). In-memory via `CaffeineBucketRegistry`; the `BucketRegistry` interface is the Redis seam for when the app runs >1 instance. When adding an endpoint that needs a non-default limit, add a tier to `RateLimitProperties` and branch on it in `RateLimitFilter` — don't scatter limiting logic elsewhere.
...
`.env` holds `DATABASE_URL`, `DB_USERNAME`, `DB_PASSWORD`, `JWT_SECRET`, `CORS_ALLOWED_ORIGINS` (comma-separated browser origins; dev defaults to `http://localhost:3000`), `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `MAIL_HOST`, `MAIL_PORT`, `MAIL_USERNAME`, `MAIL_PASSWORD`, `SPRING_PROFILES_ACTIVE`. Loaded natively via `spring.config.import=optional:file:.env[.properties]` in `application.properties`. The `.env` file is gitignored — create it locally from `.env.example`. Never commit real secrets.
